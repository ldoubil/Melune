...
class JustAudioMediaKit extends JustAudioPlatform {
  ...
  static Future<void> applyAudioFilter(String filter) async {
    audioFilter = filter;
    final instance = JustAudioPlatform.instance;
    if (instance is! JustAudioMediaKit) {
      return;
    }
    for (final player in instance._players.values) {
      await player.applyAudioFilter(filter);
    }
  }
