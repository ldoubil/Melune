...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    _player = Player(
      ...
      vo: 'null',
      // 音频版 libmpv 没有 osc 属性；media_kit 在 osc:false 时会 setProperty 并刷这条 error。
      osc: true,
      ...
      ready: () => unawaited(_onNativeReady()),
    ));
  ...
  /// media_kit 强制 `cache-on-disk=yes`，音频版 libmpv / Windows 临时目录经常建不出 lavf 文件缓存。
  /// 改走内存缓存，并把网络超时拉长，避免刚开播就失败。
  Future<void> _onNativeReady() async {
    try {
      await setProperty(_player, 'cache-on-disk', 'no');
      await setProperty(_player, 'cache', 'yes');
      await setProperty(_player, 'network-timeout', '60');
      final filter = JustAudioMediaKit.audioFilter;
      await setProperty(_player, 'af', filter);
    } catch (_) {}
